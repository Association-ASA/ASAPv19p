...
Else 
	...
	If ($DateduJour>=$Pivot)
		$AnCourantN:=Num:C11(Year of:C25(Current date:C33)+1)
	Else 
		$AnCourantN:=Year of:C25(Current date:C33)
	End if 
	<>AnCourantDevisRGN:=$AnCourantN
	<>AnCourantDevisRGA:=String:C10(<>AnCourantDevisRGN)
	//$AnCourantN:=Année de(Date du jour)
	//$AnProchainN:=$AnCourantN+1
	//$AnCourantA:=Chaîne($AnCourantN)
	//$AnProchainA:=Chaîne($AnProchainN)
	//$Test:=$AnProchainA+"G11@"  // "G11" = [Prestations]CodePrestation GEL
	//$L:=Chercher dans tableau(<>TbCombinePrestation;$Test)
	//<>AnCourantDevisRGN:=$AnCourantN+Num($L>0)
	//<>AnCourantDevisRGA:=Chaîne(<>AnCourantDevisRGN)
