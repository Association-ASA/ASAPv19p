...
$L:=Find in array:C230(<>TbPerUUID;$UUIDPersonneFacture)
If ($L<0)
	CodeBeneficiaire:=""
	VarLaboratoire:=False:C215
Else 
	If (<>TbPerTypePersonne{$L}="Laboratoire")
		CodeBeneficiaire:=ASAPTrouveNumeroLaboParUUID ($UUIDPersonneFacture)
		VarLaboratoire:=True:C214
	Else 
		$Mdp:=<>TbPerIdentificateur{$L}
		CodeBeneficiaire:=String:C10(RAEMADemoduleMdPWeb ($Mdp;True:C214))
		VarLaboratoire:=False:C215
	End if 
End if 
  //CodeBeneficiaire:=ASAPTrouveNumeroLaboParUUID ($UUIDPersonneFacture)
  ...
  //$Mdp:=<>TbPerIdentificateur{$L}
  //CodeBeneficiaire:=Chaîne(RAEMADemoduleMdPWeb ($Mdp;Vrai))
  //Sinon 
  //Laboratoire:=Vrai
  //Fin de si 
