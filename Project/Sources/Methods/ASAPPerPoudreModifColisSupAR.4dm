...
If ($L>0)
	VarNumPersonne:=RAEMADemoduleMdPWeb (<>TBPERIDENTIFICATEUR{$L};True:C214)
End if 
...
Case of 
	: ($NbreColisARCDemandé>$NbColisARCExistant)  // il veut ajouter des colis sup
		...
		For ($Salé;1;$NbNumColisSupAFaire)
			  //ALLER À ENREGISTREMENT([CampagneParticipations];$NumEnrLaboRef)
			  //OB FIXER([CampagneParticipations]Arguments;"NbreColisSupAvecRapport";$NbreColisARCDemandé)
			  //STOCKER ENREGISTREMENT([CampagneParticipations])
			  //DUPLIQUER ENREGISTREMENT([CampagneParticipations])
			  //[CampagneParticipations]UUID:=Generer UUID
	...
	: ($NbreColisARCDemandé<$NbColisARCExistant)  // il veut supprimer des colis sup
		...
		QUERY:C277([CampagneParticipations:17];[CampagneParticipations:17]NumCampagne:3=$CampagneCourante;*)
		QUERY:C277([CampagneParticipations:17]; & [CampagneParticipations:17]UUIDPersonne:2=$UUIDPersonne)
		OB REMOVE:C1226([CampagneParticipations:17]Arguments:5;"NbreColisSupAvecRapport")
		SAVE RECORD:C53([CampagneParticipations:17])
		QUERY:C277([CampagneParticipations:17];[CampagneParticipations:17]NumCampagne:3=$CampagneCourante;*)
		QUERY:C277([CampagneParticipations:17]; & [CampagneParticipations:17]UUIDPersonne:2=$UUIDPersonne;*)
		QUERY BY ATTRIBUTE:C1331([CampagneParticipations:17]; & ;[CampagneParticipations:17]Arguments:5;"CodeRaemaLié";#;Null:C1517)
