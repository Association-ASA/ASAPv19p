...
OB REMOVE:C1226([Personnes:12]Arguments:7; "EmailLivraison")
...
For ($Salé; 1; $FT)
	...
	End if 
	If (TbMailLivraison{$Salé})
		$MailLivraison:=$MailLivraison+","+$MailCourant
	End if 
End for 
// TbPerMel{LigneClic}:=""
...
If (Length:C16($MailCompta)>0)
	...
	//Fin de si 
End if 
If (Length:C16($MailLivraison)>0)
	OB SET:C1220([Personnes:12]Arguments:7; "EmailLivraison"; Substring:C12($MailLivraison; 2))
	//Si (TbPerMel{LigneClic}="")
	// TbPerMel{LigneClic}:=Remplacer chaîne($MailCompta;",";"")  // actualisation du TbPerMel
	//Fin de si 
End if 
...
//Si (OnAChangéParticipation)
//$Prc:=Exécuter sur serveur("ASAPFabriqueTbParticipations";0)
//OnAChangéParticipation:=Faux
//Fin de si 
...
If ([Personnes:12]ChampRecherche:6#VarChampRecherche)
	VarModifPer:=True:C214
End if 
If (OnAChangéParticipation)
	VarModifPar:=True:C214
End if 
...
//$H2:=Nombre de millisecondes
//FIXER TEXTE DANS CONTENEUR(chaine($H2-$H))