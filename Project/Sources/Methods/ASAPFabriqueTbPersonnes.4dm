...
If (<>ZClient) & (Count parameters:C259=0)
	$Pr:=Execute on server:C373($NomMéthode;0;"FabriquePersonnes";*)
Else 
	...
	SELECTION TO ARRAY:C260([PersonnesAdresses:11]CodePostal:6;$TbCodePostal;[PersonnesAdresses:11]LigneAdresse1:3;$TbLigneAdresse1;[PersonnesAdresses:11]LigneAdresse2:4;$TbLigneAdresse2;[PersonnesAdresses:11]LigneAdresse3:5;$TbLigneAdresse3;[PersonnesAdresses:11]LigneAdresse4:10;$TbLigneAdresse4;[PersonnesAdresses:11]Pays:8;$TbPays;[PersonnesAdresses:11]TypeAdresse:9;$TbTypeAdresse;[PersonnesAdresses:11]UUIDPersonne:2;$TbUUIDPersonne;[PersonnesAdresses:11]Ville:7;$TbVille)
	...
	ARRAY TEXT:C222(<>TbPerAdrFacAd4;$FT)
	...
	ARRAY TEXT:C222(<>TbPerAdrPerAd4;$FT)
	...
	ARRAY TEXT:C222(<>TbPerAdrProAd4;$FT)
	...
	ARRAY TEXT:C222(<>TbPerAdrLivAd4;$FT)
	...
	For ($Salé;1;$FT)
		...
		While ($L>0)
			...
			$Pt:=Get pointer:C304("<>TbPerAdr"+Substring:C12($TypeAdresseCourante;1;3)+"Ad4")
			$Pt->{$Salé}:=$TbLigneAdresse4{$L}
	...
	If ($MCE)
		...
		For ($Salé;1;$FT)
			For ($Fumé;1;$SFT)
				...
				Else 
					RESOLVE POINTER:C394(<>TbPtPersonnes{$Fumé};$nomVar;$NumTable;$numChamp)
					If ($nomVar="<>TbPARemarques")
						$Rapport:=$Rapport+Replace string:C233(<>TbPtPersonnes{$Fumé}->{$Salé};<>ZCR;" ")+$Sep
					Else 
