...
$Sujet:="Liste des numéros d'échantillons pour la série n°"+String:C10($1)+" du RAEMA n°"+TbNR{TbNR}
...
$email.attachments:=New collection:C1472(MAIL New attachment:C1644($CDPJ))
$status:=$transporter.send($email)  // Envoi du message
$MailOK:=($status.success)
If ($MailOK)  // L'envoi a été effectué
	ALERT:C41("Les mails contenant les numéros d'échantillon de la campagne "+TbNR{TbNR}+" ont été envoyés avec succès")
Else 
	ALERT:C41("Les mails contenant les numéros d'échantillon de la campagne "+TbNR{TbNR}+" n'ont pas été envoyés avec succès")
End if 