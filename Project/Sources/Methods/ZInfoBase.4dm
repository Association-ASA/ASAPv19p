...
<>PermCheDropBox:=Folder:C1567(fk dossier bureau:K87:19).folder("Dropbox").platformPath
$ok:=Folder:C1567(fk dossier bureau:K87:19).folder("Dropbox").exists
...
Else 
	...
	<>ZIPbase:=""
	$racine:=Folder:C1567(fk dossier base de données:K87:14).platformPath+"pages_Web"+Séparateur dossier:K24:12  // .dossier("pages_Web")
	WEB SET ROOT FOLDER:C634($Racine)
	...
	//$folder.create()
	//End if 
	//<>PermCheDosPdf:=$folder.platformPath
	...
	If ($ok)
		...
		<>PermCheDosRapIni:=<>PermCheDropBox+"synchrobases"+Séparateur dossier:K24:12+"Dossier Rapports"+<>ZSéparateur
