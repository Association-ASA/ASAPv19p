...
C_BOOLEAN:C305($SansPathogene; $CorrespondancePots)
...
If (<>ZClient) & (Count parameters:C259=0)
	$Prc:=Execute on server:C373($NomMéthodeCourante; 0; "FabriqueParticipations"; *)
Else 
	...
	QUERY:C277([CampagneParticipations:17];  & [CampagneParticipations:17]UUIDPersonne:2#"20202020202020202020202020202020"; *)
	...
	For ($Fumé; 1; $TT)  // autant que de campagne
		...
		For ($Salé; 1; $FT)
			...
			$CorrespondancePots:=OB Get:C1224($ArgumentCourant; "CorrespondanceEntrePots"; Est un booléen:K8:9)
			APPEND TO ARRAY:C911(<>TbLotoCorrespondancePots; $CorrespondancePots)
			If ($Fumé+$Salé=2)
				APPEND TO ARRAY:C911($TbPropriétéArgument; "CorrespondanceEntrePots")
			End if 
			...
			$L:=Find in array:C230($TbNomGerme; "BACILLUS CEREUS")
			If ($L<0)
			...
			$L:=Find in array:C230($TbNomGerme; "BACTERIES LACTIQUES")
			If ($L<0)
			...
			$L:=Find in array:C230($TbNomGerme; "LEVURES-MOISISSURES ENSEMBLE")
			If ($L<0)
			...
			$L:=Find in array:C230($TbNomGerme; "LEVURES-MOISISSURES SEPAREMENT")
			If ($L<0)
			...
			$L:=Find in array:C230($TbNomGerme; "CAMPYLOBACTER DENOMBREMENT")
			If ($L<0)
	...
	If ($MCE)  // Il demande à mettre le tableau dans le PP
		...
		$ET:=$ET+<>ZTab+"<>TbLotoSansPathogene"+<>ZTab+"<>TbLotoNumBonCommandePoudre"+<>ZTab+"<>TbLotoProformaPoudre"+<>ZTab+"<>TbLotoCodeRaemaLié"+<>ZTab+"<>TbLotoNbColisPoudreSR"+<>ZTab+"<>TbLotoNbColisPoudreAR"+<>ZTab+"<>TbLotoCorrespondancePots"
		...
		$ET:=$ET+<>ZTab+"NbreColisSupSansRapport"+<>ZTab+"NbreColisSupAvecRapport"+<>ZTab+"CorrespondanceEntrePots"+<>ZTab+"NumBonCommandeGel"+<>ZTab+"RGProforma"+(22*(<>ZTab+""))+<>ZCR
		...
		$ET:=$ET+<>ZTab+"Sans pathogène"+<>ZTab+"Num Bon Commande Poudre"+<>ZTab+"Proforma Poudre"+<>ZTab+"Code Raema Lié"+<>ZTab+"Nb Colis Poudre SR"+<>ZTab+"Nb Colis Poudre AR"+<>ZTab+"Correspondance pots"
		...
		For ($Salé; 1; $FT)
			...
			$Rapport:=$Rapport+String:C10(<>TbLotoSansPathogene{$Salé})+<>ZTab+<>TbLotoNumBonCommandePoudre{$Salé}+<>ZTab+<>TbLotoProformaPoudre{$Salé}+<>ZTab+<>TbLotoCodeRaemaLié{$Salé}+<>ZTab+String:C10(<>TbLotoNbColisPoudreSR{$Salé})+<>ZTab+String:C10(<>TbLotoNbColisPoudreAR{$Salé})+<>ZTab+String:C10(<>TbLotoCorrespondancePots{$Salé})+<>ZTab
