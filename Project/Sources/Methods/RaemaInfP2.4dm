...
If ($OC) & ($AuMoinsUn)
	...
	$FT:=Size of array:C274($TbEchSaisis)  // colonnes
	$SFT:=Size of array:C274(TbNomGermeConta)  //lignes
	For ($Sec;1;$FT)
		$THTML:=$THTML+"      <td class="+<>ZGuil+"hauttbech"+<>ZGuil+">"+$TbEchSaisis{$Sec}+"</td>"+<>ZCR
	End for 
	$THTML:=$THTML+"    </tr>"+<>ZCR
	$Pair:=True:C214
	For ($Fumé;1;$SFT)
		...
		For ($Salé;1;$FT)
			$NuméroRang:=Num:C11(Substring:C12($TbEchSaisis{$Salé};$Rang;1))
			Case of 
				: ($NuméroRang=0) | ($NuméroRang=1)
					$Ligne:=1
					
				: ($NuméroRang=2) | ($NuméroRang=3)
					$Ligne:=2
					
				: ($NuméroRang=4) | ($NuméroRang=5)
					$Ligne:=3
					
				: ($NuméroRang=6) | ($NuméroRang=7)
					$Ligne:=4
					
				: ($NuméroRang=8) | ($NuméroRang=9)
					$Ligne:=5
			End case 
			
			$Pt:=Get pointer:C304("TbSerie"+String:C10($Ligne)+"Conta")
