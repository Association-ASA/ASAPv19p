...
//$Pt:=OBJET Lire pointeur(Objet nommé;"CacCA")
//$Pt->:=Num(OB Lire([Personnes]Arguments;"CA";Est un booléen))
CacCA:=0
CacBureau:=0
CacAD:=0
CacADG:=0
...
//$Pt:=OBJET Lire pointeur(Objet nommé;"CacAD")
//$Pt->:=Num(OB Lire([Personnes]Arguments;"AsaDia";Est un booléen))

//$Pt:=OBJET Lire pointeur(Objet nommé;"CacADG")
//$Pt->:=Num(OB Lire([Personnes]Arguments;"AsaDia Gibier";Est un booléen))
...
SELECTION TO ARRAY:C260([Ventes:16]NumFacture:2; TbNumFacture; [Ventes:16]DateVente:4; TbDateF; [Ventes:16]Intitule:3; TbIntituleF; [Ventes:16]PrixTotalHT:7; TbPrixF; [Ventes:16]DateReglement:12; $TbDateRèglement)
LONGINT ARRAY FROM SELECTION:C647([Ventes:16]; TbNumEnrF)

$FT:=Size of array:C274(TbNumFacture)
ARRAY TEXT:C222(TbPayeF; $FT)
SORT ARRAY:C229(TbNumFacture; TbDateF; TbIntituleF; TbPrixF; TbNumEnrF; TbPayeF; $TbDateRèglement; <)
