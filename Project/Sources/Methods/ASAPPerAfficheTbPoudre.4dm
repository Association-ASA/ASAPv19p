...
C_BOOLEAN:C305($CorrespondancePots)
$Mois:=$1
...
If ($TT>0)  // Il existe au moins 1 participation
	...
	// Le bon de commande
	VarBdCPoudre:=OB Get:C1224([CampagneParticipations:17]Arguments:5; "NumBonCommandePoudre"; Est un texte:K8:3)
	...
	// la correspondance entre les pots
	$CorrespondancePots:=OB Get:C1224([CampagneParticipations:17]Arguments:5; "CorrespondanceEntrePots"; Est un booléen:K8:9)
	$Pt:=Get pointer:C304("CaC"+$Mois+"CP")
	$Pt->:=Num:C11($CorrespondancePots)
	...
	If ($Pt->="DHL@")
		$PtSuivi:=Get pointer:C304("Var"+$Mois+"NumSuiviDHL")
		$PtSuivi->:=OB Get:C1224([CampagneParticipations:17]Arguments:5; "NumSuiviDHL"; Est un texte:K8:3)
		If ($Mois="Mars")
			OBJECT SET VISIBLE:C603(*; "NumSuiviDHLMars@"; True:C214)
		Else 
			OBJECT SET VISIBLE:C603(*; "NumSuiviDHLOctobre@"; True:C214)
		End if 
	Else 
		$PtSuivi:=Get pointer:C304("Var"+$Mois+"NumSuiviDHL")
		$PtSuivi->:=""
		If ($Mois="Mars")
			OBJECT SET VISIBLE:C603(*; "NumSuiviDHLMars@"; False:C215)
		Else 
			OBJECT SET VISIBLE:C603(*; "NumSuiviDHLOctobre@"; False:C215)
		End if 
	End if 
