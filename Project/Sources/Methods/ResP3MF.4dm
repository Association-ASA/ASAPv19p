...
If (<>TbLotoNumLaboRapportCampagne{$L}#"")
	$pos:=Position:C15("_";<>TbLotoNumLaboRapportCampagne{$L})
	NumLaboActuelA:=Substring:C12(<>TbLotoNumLaboRapportCampagne{$L};1;($pos-1))
	ARRAY TEXT:C222($TbItem;0)
	ZTexteVersTableau (<>TbLotoIdentifiantParticipation{$L};->$TbItem;"_")
	MotDePasseActuel:=$TbItem{3}
Else 
...
If (ResP3ChasseLaboFantomes )  // Le mdp caché est correct
	...
	If (Find in array:C230(TbNomsP3;"submit2")>0)  // il ne demande qu'à enregistrer
		...
		If (LangueCourante="F") | (LangueCourante="")
