...
If (Count parameters:C259>7)
	...
	For ($i; 0; $length)
		...
		Case of 
			: (Value type:C1509($1.value[$8[$i]])=Est une collection:K8:32)
				$txt:=JSON Stringify:C1217($1.value[$8[$i]]; *)
			: (Value type:C1509($1.value[$8[$i]])=Est un objet:K8:27)
				$txt:=JSON Stringify:C1217($1.value[$8[$i]]; *)
			: (Value type:C1509($1.value[$8[$i]])=Est un pointeur:K8:14)
				Case of 
					: (Value type:C1509($1.value[$8[$i]]->)=Est une collection:K8:32)
						$txt:=JSON Stringify:C1217($1.value[$8[$i]]->; *)
					: (Value type:C1509($1.value[$8[$i]]->)=Est un objet:K8:27)
...
Else 
	...
	Case of 
		: (Value type:C1509($1.value)=Est une collection:K8:32)
