...
$ProchNumCampagne:=<>NumCampagneEnCoursSaisie  // Numéro de prochain RAEMA : <>NumCampagneEnCoursSaisie  (RAEMAInitPUMCampagne)
...
$ParticipeDerCampagne:=($L>0)
OB SET:C1220($Objet;"Participe";$ParticipeDerCampagne)
...
NiveauConta:=OB Get:C1224([RAEMACampagnes:20]Arguments:7;"Niveaux de contamination";Est un texte:K8:3)


  //$CodePériode:=1+Num(<>NumCampagneEnCoursSaisie=<>NumCampagneEnCoursInfoOK)+Num($RapportDispo)
  //$CodePériode:=1+$Niveau[[1]]#<>ZTAB+Num($RapportDispo)
  //$CodePériode:=1+Num(<>NumCampagneEnCoursSaisie=<>NumCampagneEnCoursInfoOK)+Num($RapportDispo)

$CodePériode:=1+Num:C11(NiveauConta#"")+Num:C11($RapportDispo)
OB SET:C1220($Objet;"CodePériode";$CodePériode)
$OuEnSommesNous:=WebEspaceHTMLOuEnSommesNous ($Objet;$Langue;$UUIDLabo)
...
$THTML:=$THTML+"            <div class="+<>ZGuil+"LienAcces"+<>ZGuil+">"+<>ZCR
If ($Langue="A")
	$THTML:=$THTML+"              <a href="+<>ZGuil+"w2raematarifsA.shtml"+<>ZGuil+">"
Else 
	$THTML:=$THTML+"              <a href="+<>ZGuil+"w2raematarifs.shtml"+<>ZGuil+">"
End if 
$THTML:=$THTML+"                <img src="+<>ZGuil+"images/IconeTarifs.png"+<>ZGuil+" alt="+<>ZGuil+"Tarifs"+<>ZGuil
$THTML:=$THTML+" width="+<>ZGuil+"30px"+<>ZGuil+"/><br />"+<>ZCR
$THTML:=$THTML+"                <br />"+WebTraduitTexte ("Consulter les tarifs RAEMA";$Langue)+"<br />"+<>ZCR
$THTML:=$THTML+"              </a>"
$THTML:=$THTML+"            </div>"+<>ZCR2
...
$THTML:=$THTML+"          <div class="+<>ZGuil+"LienAcces"+<>ZGuil+">"+<>ZCR
$THTML:=$THTML+"            <a href="+<>ZGuil+"pdf/BoiteIsoRaemaFr.pdf"+<>ZGuil+">"
...
$THTML:=$THTML+"          <div class="+<>ZGuil+"LienAcces"+<>ZGuil+">"+<>ZCR
$THTML:=$THTML+"            <a href="+<>ZGuil+"RaemaInfP1"+$Langue+$UUIDLabo+<>ZGuil+">"
$THTML:=$THTML+"              <img src="+<>ZGuil+"images/NiveauConta.png"+<>ZGuil+" alt="+<>ZGuil+"Niveau de contamination"+<>ZGuil+" width="+<>ZGuil+"35px"+<>ZGuil+"/><br />"+<>ZCR
$THTML:=$THTML+"              <br />"+WebTraduitTexte ("Niveau de contamination";$Langue)+"<br />"+<>ZCR
$THTML:=$THTML+"            </a>"
$THTML:=$THTML+"          </div>"+<>ZCR2
...
If ($ParticipeDerCampagne)
	$THTML:=$THTML+"          <div class="+<>ZGuil+"LienAcces"+<>ZGuil+">"+<>ZCR
	
	Case of 
			  // Colis sans pathogène
		: ($SansPathogène)
			$THTML:=$THTML+"            <a href="+<>ZGuil+"pdf/LettreAccompSansPathoP.pdf"+<>ZGuil+">"
			
			  // Colis supp
		: ((($ColisSuppSR) | ($ColisSuppAR) | ($PotsSupp))>0)
			$THTML:=$THTML+"            <a href="+<>ZGuil+"pdf/LettreAccompColisSupP.pdf"+<>ZGuil+">"
			
		Else 
			  // Colis normal
			$THTML:=$THTML+"            <a href="+<>ZGuil+"pdf/LettreAccompNormalP.pdf"+<>ZGuil+">"
...
If (<>TbPAURLProforma{$LigneTbGnx}#"") & ($Participe=True:C214)
	  //$ChemCible:=<>PermCheDosProforma+NumLaboActuelA+"_"+$UUIDLabo+Séparateur dossier
	  //$DocProforma:=$ChemCible+"ProformaRAEMA.pdf"
	$URLProforma:="https://association.asa-spv.fr/pdf/Proforma/"+NumLaboActuelA+"_"+$UUIDLabo+"/ProformaRAEMA.pdf"
	
	$THTML:=$THTML+"            <div class="+<>ZGuil+"LienAcces"+<>ZGuil+">"+<>ZCR
	$THTML:=$THTML+"              <a href="+<>ZGuil+$URLProforma+<>ZGuil+">"
	$THTML:=$THTML+"                <img src="+<>ZGuil+"images/IconeFacDouan.png"+<>ZGuil+" alt="+<>ZGuil+"Facture douanière"+<>ZGuil+" width="+<>ZGuil+"30px"+<>ZGuil+"/><br />"+<>ZCR
	$THTML:=$THTML+"                <br />"+WebTraduitTexte ("Télécharger votre facture douanière";$Langue)+"<br />"+<>ZCR
	$THTML:=$THTML+"              </a>"
	$THTML:=$THTML+"            </div>"+<>ZCR2
End if 
...
ARRAY TEXT:C222($TbNumCampagne;4)
  //$Moinsse:=-Num($CodePériode#3)  // si la campagne actuelle a produit son rapport, on se base sur elle pour avoir les 3 autres
$Moinsse:=0
...
For ($Salé;1;4)
	...
	$THTML:=$THTML+"                  </td>"+<>ZCR
	  ...
	  // Le rapport général
	$THTML:=$THTML+"                  <td>"+<>ZCR
	$CD:=<>PermCheDosPdf+"RG"+$TbNumCampagne{$Salé}+"_"+$Langue+".pdf"
	If (Test path name:C476($CD)=Est un document:K24:1)
		$THTML:=$THTML+"                    <a href="+<>ZGuil+"PDF/RG"+$TbNumCampagne{$Salé}+"_"+$Langue+".pdf"+<>ZGuil+">"+<>ZCR
...
$RaemaAnnuel1:=RaemaTrouveNumCampagneParAn (Year of:C25(Current date:C33);1)
  ...
  //$L:=Chercher dans tableau(<>TbLotoIdentifiantParticipation;$Test)
QUERY:C277([CampagneParticipations:17];[CampagneParticipations:17]NumCampagne:3=$RaemaAnnuel1;*)
QUERY:C277([CampagneParticipations:17]; & [CampagneParticipations:17]UUIDPersonne:2=$UUIDLabo)
$L:=Records in selection:C76([CampagneParticipations:17])
...
$RaemaAnnuel2:=RaemaTrouveNumCampagneParAn (Year of:C25(Current date:C33);2)
  ...
  //$L:=Chercher dans tableau(<>TbLotoIdentifiantParticipation;$Test)
QUERY:C277([CampagneParticipations:17];[CampagneParticipations:17]NumCampagne:3=$RaemaAnnuel2;*)
QUERY:C277([CampagneParticipations:17]; & [CampagneParticipations:17]UUIDPersonne:2=$UUIDLabo)
$L:=Records in selection:C76([CampagneParticipations:17])
...
$RaemaAnnuel3:=RaemaTrouveNumCampagneParAn (Year of:C25(Current date:C33);3)
  ...
  //$L:=Chercher dans tableau(<>TbLotoIdentifiantParticipation;$Test)
QUERY:C277([CampagneParticipations:17];[CampagneParticipations:17]NumCampagne:3=$RaemaAnnuel3;*)
QUERY:C277([CampagneParticipations:17]; & [CampagneParticipations:17]UUIDPersonne:2=$UUIDLabo)
$L:=Records in selection:C76([CampagneParticipations:17])
...
$RaemaAnnuel4:=RaemaTrouveNumCampagneParAn (Year of:C25(Current date:C33);4)
  ...
  //$L:=Chercher dans tableau(<>TbLotoIdentifiantParticipation;$Test)
QUERY:C277([CampagneParticipations:17];[CampagneParticipations:17]NumCampagne:3=$RaemaAnnuel4;*)
QUERY:C277([CampagneParticipations:17]; & [CampagneParticipations:17]UUIDPersonne:2=$UUIDLabo)
$L:=Records in selection:C76([CampagneParticipations:17])
