...
If ($Lon_Parameters>0)
	...
	If (Length:C16($Txt_Path)=0)
		$Txt_Path:=Get 4D folder:C485(_o_Dossier Extras:K5:12)+"regex.xml"
	End if 
	
	OK:=Num:C11(Test path name:C476($Txt_Path)=Est un document:K24:1)
	...
	If (OK=1)
		...
		$Lon_Error:=REGEX_SubstituteText("\\s*\\(\\?#[^)]*\\)|\\s"; ""; ->$Txt_Buffer)
	End if 
End if 
