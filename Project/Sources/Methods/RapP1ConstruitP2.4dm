...
$THTML:=$THTML+"<p class="+<>ZGuil+"hautpage bleu"+<>ZGuil+">TELECHARGEMENT DES "+$TitrePage+" </font> </p>"+<>ZCR
Case of 
	...
	: (TypeDemande="Rap") | (TypeDemande="Att")
		...
		//$DocEstLa:=Vrai
		$THTML:=$THTML+"  <br /><br /><p class="+<>ZGuil+"textecentre xdtitrelivre"+<>ZGuil+">"
	...
	: (TypeDemande="Fac")
		...
		//$THTML:=$THTML+"  <p>&nbsp;</p> <p>&nbsp;</p>"+◊ZCR
		$THTML:=$THTML+"  <p class="+<>ZGuil+"hautpage"+<>ZGuil+">"
		$THTML:=$THTML+"    <a href="+<>ZGuil+"PDF/RIBAsa.pdf"+<>ZGuil+">Télécharger le relevé d'identité bancaire de l'ASA</a>"
		$THTML:=$THTML+"  </p><br /><br />"+<>ZCR
		$THTML:=$THTML+"  <p class="+<>ZGuil+"hautpage"+<>ZGuil+">"+<>ZCR  // ouverture du paragraphe
		//$THTML:=$THTML+"<p class="+<>ZGuil+"xdtitrelivre"+<>ZGuil+">Rapport disponibles</p>"+<>ZCR
		$FT:=Size of array:C274($PtTbListeDocuments->)
		For ($Salé; 1; $FT)
			...
			$NumFactureInt:=Substring:C12($FichierCourant; 1; $Pos-1)
			$NumFacture:=Replace string:C233($NumFactureInt; "_"; "/")
			QUERY:C277([Factures:15]; [Factures:15]NumFacture:3=$NumFacture)
			If ([Factures:15]Regle:12=True:C214) & ($NumFacture#"@D@") & ($NumFacture#"@A@")
				$PayéOuPas:="<span style="+<>ZGuil+"color : #2DE939"+<>ZGuil+"<b>    PAYÉE</b></span>"
			Else 
				$PayéOuPas:="<span style="+<>ZGuil+"color : #E9322D"+<>ZGuil+"<b>    NON PAYÉE</b></span>"
			End if 
			If ($NumFacture="@D@") | ($NumFacture="@A@")
				$PayéOuPas:="<span style="+<>ZGuil+"color : #A3A3A3"+<>ZGuil+"<b>    NON CONCERNÉE</b></span>"
			End if 
			...
			If ($Pos>0)  // Elimination des fichiers autres que dossier
				...
				$THTML:=$THTML+"  <a href="+<>ZGuil+"PDF/Factures/"+$NumAdhA+"/"+$FichierCourant+<>ZGuil+"> "+$FichierCourant+"</a>"+$PayéOuPas+"<p/><br />"+<>ZCR
			End if 
		End for 
		$THTML:=$THTML+"   <br /><br /><br /><br />"+<>ZCR
		$THTML:=$THTML+"  <p class="+<>ZGuil+"hautpage bleu"+<>ZGuil+">"
