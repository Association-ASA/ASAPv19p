...
$THTML:=""
$Cachée:=Num:C11(Escamote & ($ClassSelectCache#""))*(" style="+<>ZGuil+"display: none;"+<>ZGuil)
$THTML:=$THTML+"           <tr class="+<>ZGuil+"ligne"+$ClassePaireImpaire2+" "+$ClassSelectCache+<>ZGuil+$Cachée+">"+<>ZCR
...
If (<>TbMultiple{$compteur}="M")  // PUM
	...
	For ($Salé; 1; $FT)
		...
		Else 
			$Selectionné:=""
		End if 
		If (Escamote=False:C215)
			Escamote:=(($Selectionné#"") & ($ClassOption#""))  // On a selectionné une ligne qui entraine l'escamotage
		End if 
