Case of 
	: (Form event code:C388=Sur chargement:K2:1)
		// Déclaration des c@g
		C_LONGINT:C283($NumTable; $LargeurColonneTotale; $Salé; $FT; $NumProchaineLigneTbOb; $LargeurColonneCourante; $HauteurPapier; $LargeurPapier; $AffEntete)
		...
		$NumTable:=OB Get:C1224(TbObParam{1}; "LBSource"; Est un entier long:K8:6)
		LISTBOX SET TABLE SOURCE:C1013(*; "LB_ListeImpression"; $NumTable)
		...
		If ($CoeffLargeur#1)  // largeur modifiée
			...
			If ($TaillePolice<1)  // On s'arrête à 1 !!!
				$TaillePolice:=1
			End if 
			OBJECT SET FONT SIZE:C165(*; "LB_ListeImpression"; $TaillePolice)  // Attribution de la taille de police
			// 4D n'ajuste pas la hauteur de ligne si on ne la fixe pas à nouveau après avoir modifié la taille de la police
			LISTBOX SET ROWS HEIGHT:C835(*; "LB_ListeImpression"; 1; lk lignes:K53:23)  // Attribution de la hauteur de ligne
		End if 
			...
			Else   // Ca tient ou la colonne actuelle ne peut pas tenir dans la largeur papier : on imprime
				...
				If ($vPPNom#"")  // Pas de pied
					If ($vColFormula="")  // Pas de pied, Pas de formule
						LISTBOX INSERT COLUMN:C829(*; "LB_ListeImpression"; $Salé; $ColNom; $PtColVar->; $ETNom; $PtColonneVide; $vPPNom; $PtColonneVide)
					Else   // Pas de pied, Une formule
						...
						LISTBOX INSERT COLUMN FORMULA:C970(*; "LB_ListeImpression"; $Salé; $ColNom; $vColFormula; $type; $ETNom; $PtColonneVide; $vPPNom; $PtColonneVide)
					End if 
				Else   // Un pied
					If ($vColFormula="")  // Un pied, Pas de formule
						LISTBOX INSERT COLUMN:C829(*; "LB_ListeImpression"; $Salé; $ColNom; $PtColVar->; $ETNom; $PtColonneVide)
					Else   // Un pied, Une formule
						$type:=OB Get:C1224(TbObParam{$Salé}; "colType"; Est un entier long:K8:6)  // Type du résultat de la formule
						LISTBOX INSERT COLUMN FORMULA:C970(*; "LB_ListeImpression"; $Salé; $ColNom; $vColFormula; $type; $ETNom; $PtColonneVide)
					End if 
				End if 
