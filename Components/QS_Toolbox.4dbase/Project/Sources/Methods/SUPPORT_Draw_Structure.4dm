...
// created 20/4/09 by Vincent de Lachaux
// ----------------------------------------------------
// modified by Vincent de Lachaux(21/4/9)
// ----------------------------------------------------
// declarations
...
C_LONGINT:C283($Lon_i; $Lon_j; $Lon_maxHeight; $Lon_maxWidth; $Lon_parameters)
C_LONGINT:C283($Lon_type; $Lon_x)
C_PICTURE:C286($Pic_buffer)
C_REAL:C285($Num_destinationHeight; $Num_destinationMiddle; $Num_destinationWidth; $Num_destinationX; $Num_destinationY)
C_REAL:C285($Num_height; $Num_left; $Num_middle; $Num_middleOffset; $Num_offsetX)
C_REAL:C285($Num_sourceHeight; $Num_sourceMiddle; $Num_sourceWidth; $Num_sourceX; $Num_sourceY)
C_REAL:C285($Num_top; $Num_viaPointX; $Num_width; $Num_X1; $Num_X2)
C_REAL:C285($Num_y1; $Num_y2)
C_TEXT:C284($Dom_arrow; $Dom_buffer; $Dom_circle; $Dom_color; $Dom_coordinates)
C_TEXT:C284($Dom_defs; $Dom_destination; $Dom_field_extra; $Dom_fieldName; $Dom_fieldRect)
C_TEXT:C284($Dom_fieldType; $Dom_marker; $Dom_relation; $Dom_root; $Dom_shadow)
C_TEXT:C284($Dom_source; $Dom_structure; $Dom_svg; $Dom_table_extra; $Dom_table_ref)
C_TEXT:C284($Dom_tableGroup; $Dom_tableName; $Dom_tableRect; $Dom_x; $Txt_attributeName)
C_TEXT:C284($Txt_attributeValue; $Txt_blue; $Txt_color; $Txt_data; $Txt_destinationUUID)
C_TEXT:C284($Txt_fieldColor; $Txt_fieldName; $Txt_fieldUID; $Txt_fontStyle; $Txt_fontWeight)
C_TEXT:C284($Txt_green; $Txt_kind; $Txt_path; $Txt_pictureFormat; $Txt_red)
C_TEXT:C284($Txt_sourceUUID; $Txt_tableColor; $Txt_tableName; $Txt_tableUID; $Txt_tableUUID)

ARRAY TEXT:C222($tDom_fields; 0x0000)
ARRAY TEXT:C222($tDom_relatedFields; 0x0000)
ARRAY TEXT:C222($tDom_relations; 0x0000)
ARRAY TEXT:C222($tDom_tables; 0x0000)
...
ARRAY TEXT:C222($tTxt_types; 21)
$tTxt_types{1}:="B"  // boolean
$tTxt_types{3}:="16"  // integer
$tTxt_types{4}:="32"  // longInt
$tTxt_types{5}:="64"  // INt 64Bits
$tTxt_types{6}:="0.5"  // real
$tTxt_types{7}:="F"  // float
$tTxt_types{8}:="D"  // date
$tTxt_types{9}:="H"  // hour
$tTxt_types{10}:="T"  // text
$tTxt_types{12}:="I"  // PIcture
$tTxt_types{14}:=".T"
$tTxt_types{18}:="X"  // blob
$tTxt_types{21}:="{}"  // object
		...
		If (OK=1)
			...
			If (OK=1)
				...
				If (OK=1)
					...
					$Dom_structure:=DOM Create XML element:C865($Dom_svg; "g"; "filter"; "url(#Shadow)")
					...
					For ($Lon_i; 1; Size of array:C274($tDom_tables); 1)
						...
						If (Not:C34($Boo_trashed))
							...
							$Dom_tableGroup:=DOM Create XML element:C865($Dom_structure; "g"; "id"; $Txt_tableUID)  // ;"filter";"url(#Shadow)")
						...
						If (Size of array:C274($tDom_relatedFields)>=2)
							...
							If (OK=1)
								...
								If (OK=1)
									...
									$Dom_relation:=DOM Create XML element:C865($Dom_structure; "polyline"; "fill"; "none"; "stroke"; $Txt_color; "stroke-width"; 3; "points"; $Txt_data)
									DOM SET XML ATTRIBUTE:C866($Dom_relation; "marker-start"; "url(#relation_N)"; "marker-end"; "url(#relation_1)"; "stroke-linejoin"; "round"; "stroke-opacity"; "0.8")
					...
					If ($Txt_pictureFormat="")  // return  a picture
						
						SVG EXPORT TO PICTURE:C1017($Dom_svg; $0)
						
					Else 
						
						$Txt_path:=Replace string:C233($Txt_path; ".xml"; $Txt_pictureFormat)
						
						If ($Txt_pictureFormat=".svg")  // SAve as SVG document
							
							DOM EXPORT TO FILE:C862($Dom_svg; $Txt_path)
							
						Else   // save as image file
							...
							// ajouter par PE le 27/09/2011
							READ PICTURE FILE:C678($Txt_path; $0)
