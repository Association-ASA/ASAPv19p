...
$headerHeight:=LISTBOX Get property:C917(*;$Listbox_Name;_o_lk header height:K53:5)
$footerHeight:=LISTBOX Get property:C917(*;$Listbox_Name;_o_lk footer height:K53:21)
$hsHeight:=LISTBOX Get property:C917(*;$Listbox_Name;lk hor scrollbar height:K53:7)

If (($x>$left) & ($x<$right) & ($y>($top+$headerHeight)) & ($y<($bot-$footerHeight-$hsHeight)))
	
	$pos:=LISTBOX Get property:C917(*;$Listbox_Name;_o_lk ver scrollbar position:K53:11)
...
For ($i;1;Size of array:C274($arrColNames))
	...
	If (($x>=$left) & ($x<=$right))
		...
		$columnPtr:=OBJECT Get pointer:C1124(Object named:K67:5;$arrColNames{1})  // Column pointer for size
		
		  // Clear all the Rows
		For ($k;1;Size of array:C274($arrColNames))
			For ($j;1;LISTBOX Get number of rows:C915(*;$Listbox_Name))
				LISTBOX SET ROW COLOR:C1270(*;$arrColNames{$k};$j;0x00FFFFFF;lk background color:K53:25)
			End for 
		End for 
		
		  // Highlight the particular cell
		LISTBOX SET ROW COLOR:C1270(*;$arrColNames{$i};$rowNum;0x003875D7;lk background color:K53:25)
