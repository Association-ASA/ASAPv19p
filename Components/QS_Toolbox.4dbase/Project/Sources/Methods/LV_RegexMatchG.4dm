//%attributes = {"invisible":true,"preemptive":"capable"}
  // PM: "LV_RegexMatchG" (new LV 21.05.20, 09:04:46)
  ...
  // LV_RegexMatchG (srcTxt;regexPattern{;startPos{;withPosLengthResults}}) -> resultCollection with strings or with strings+postions+lengths
  ...
  // UseExamples:
  // - $resultCollection:=LV_RegexMatchG ("abc123xy98";"\\d")  // -> ["1","2","3","9","8"]
  // - $resultCollection:=LV_RegexMatchG ("abc123xy98";"\\d+")  // -> ["123","98"]
  // - $resultCollection:=LV_RegexMatchG ("abc123xy98";"\\d+";7)  // -> ["98"]
  // - $resultCollection:=LV_RegexMatchG ("abc123xy98";"\\d+";1;True)  // -> [{strings:["123","98"]},{positions:[4,9]},{lengths:[3,2]}]
  // - $resultCollection:=LV_RegexMatchG ("Dog called happyDog";"(HAPPY){0,1}(?i)DOG")  // -> ["Dog","Dog"]
  // - $resultCollection:=LV_RegexMatchG ("Dog called happyDog";"(happy){0,1}(?i)DOG")  // -> ["Dog","happyDog"]
  // - $resultCollection:=LV_RegexMatchG ("abc123 \r\n\t xy98";"\\p{C}")  // -> ["\r","\n","\t"] // find all cntrl-codes (not only CR, LF or TAB)
  // - $resultCollection:=LV_RegexMatchG ("abc123 \r\n\t xy98";"[:cntrl:]")  // -> ["\r","\n","\t"] // find all cntrl-codes (not only CR, LF or TAB)
  // - $resultCollection:=LV_RegexMatchG ("Dog called happyDog";"(?i)(HAPPY){0,1}DOG")  // -> ["Dog","happyDog"]
  // - $resultCollection:=LV_RegexMatchG ("ABC123abc98";"abc")  // -> [abc]
  // - $resultCollection:=LV_RegexMatchG ("ABCSS123abcß98";"(?i:abcss)")  // -> [ABCSS,abcß]
  // - $resultCollection:=LV_RegexMatchG ("ABC123abc98";"(?-i:abc)")  // -> [abc]
  // - $resultCollection:=LV_RegexMatchG ("aBxy123aBxY";"(?i)abxy")  // -> [aBxy,aBxY]
  // - $resultCollection:=LV_RegexMatchG ("aBxy123aBxY";"(?i:ab)(?-i:xy)")  // -> [aBxy]
