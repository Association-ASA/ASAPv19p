...
For ($x;1;Size of array:C274($atColNames))
	If ($afVisible{$x}=True:C214)
		...
		$tLabel:=Replace string:C233(OBJECT Get title:C1068(*;$atHeadNames{$x});"\r";" ")
		  //$tLabel:=Choose(Position(" ";$tLabel)>0;$tLabel;UTIL_AddSpacesToCamelCase ($tLabel))  //add spaces to Text To Array can do its business
...
If (LISTBOX Get headers height:C1144($pLB->;lk lines:K53:23)#$lMaxLines)
	LISTBOX SET HEADERS HEIGHT:C1143($pLB->;$lMaxLines;lk lines:K53:23)
End if 