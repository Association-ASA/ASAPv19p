...
Else 
	...
	gError:=-50*Num:C11(($Lon_Groups>1) & Not:C34($Boo_Array2D))  // parameter error
	...
	If (gError=0)
			...
			If ($Boo_OK) & (gError=0)
				...
				For ($Lon_i;0;Size of array:C274($tLon_Positions);1)
					...
					If ($tLon_Positions{$Lon_i}>0)
						$Lon_Start:=$tLon_Positions{$Lon_i}+Choose:C955($tLon_Lengths{$Lon_i}>0;$tLon_Lengths{$Lon_i};1)
					End if 
		...
		If (gError=0)
			gError:=-Num:C11(Size of array:C274($4->)=0)
		End if 
