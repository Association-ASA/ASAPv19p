...
C_TEXT:C284($type)
C_TEXT:C284($id; $commandName; $theme; $constantToken)
	...
	If ($objAppPath#Null:C1517)
		If ($objAppPath.parentFolder#Null:C1517)
			If (Length:C16($objAppPath.parentFolder)>0)
				...
				If (Test path name:C476($objAppPath.parentFolder+Replace string:C233("4D.app/Contents/Resources/en.lproj/4DSyntaxEN.xlf"; "/"; Folder separator:K24:12))=Is a document:K24:1)
					$xml_Struct_Ref:=DOM Parse XML source:C719($objAppPath.parentFolder+Replace string:C233("4D.app/Contents/Resources/en.lproj/4DSyntaxEN.xlf"; "/"; Folder separator:K24:12))
				Else 
					//$xml_Struct_Ref:=DOM Parse XML source(Get 4D folder(Current resources folder)+"4D"+Folder separator+"4DSyntaxEN.xlf")
					$xml_Struct_Ref:=DOM Parse XML source:C719(Folder:C1567(fk resources folder:K87:11).file("4D/4DSyntaxEN.xlf").platformPath)
				End if 
																														...
																														: ($cmdValKind="desc@")  // command description
																															If ($lastCmdNo=$cmdNo)
																																...
																																If (($commandName#"") & ($commandName#"_4D"))
																																	...
																																	End if 
																																	$resultCol.push(New object:C1471("serialNumber"; $resultCol.length+1; "name"; $commandName; "tokenID"; $id; "type"; "command"; "theme"; $theme; "tokenName"; $contents; "treadSeafe"; $txtAttr; "syntax"; $attributes.cmdSyntax; "description"; $attributes.cmdDesc))
			...
			If (OK=1)  //command number exists
				If (Length:C16($commandName)>0)  //command is not disabled
					...
					If (($commandName#"") & ($commandName#"_4D"))
						...
						// $attributes.cmdSyntax:=$lastCmdTxt
						// $attributes.cmdDesc:=$cmdTxt
						//$resultCol.push(New object("column1";$resultCol.length+1;"column2";$commandName;"column3";$id;"column4";"command";"column5";$theme;"column6";$contents;"column7";String(Bool($threadsafe ?? 0));"column8";"";"column9";""))
						$resultCol.push(New object:C1471("serialNumber"; $resultCol.length+1; "name"; $commandName; "tokenID"; $id; "type"; "command"; "theme"; $theme; "tokenName"; $contents; "treadSeafe"; String:C10(Bool:C1537($threadsafe ?? 0)); "syntax"; ""; "description"; ""))
					End if 
				End if 
			End if 
		End if 
...
If ($withConstList)
	...
	Repeat 
		...
		Repeat 
			...
			//$constantName:=EXECUTE FORMULA("Parse formula:C1576("+$constantToken+")")
			...
			If ((Length:C16($constantName)>0) & ($constantName#("‘k"+String:C10($constantThemeNum)+";"+String:C10($constantNum)+"‘")))
				...
				End if 
				//$resultCol.push(New object("column1";$resultCol.length+1;"column2";$constantName;"column3";"K"+String($constantThemeNum)+":"+String($constantNum);"column4";"constant";"column5";$theme;"column6";$contents;"column7";$result))
				$resultCol.push(New object:C1471("serialNumber"; $resultCol.length+1; "name"; $constantName; "tokenID"; "K"+String:C10($constantThemeNum)+":"+String:C10($constantNum); "type"; "constant"; "theme"; $theme; "tokenName"; $contents; "treadSeafe"; $result; "syntax"; ""; "description"; ""))
			Else 
				Case of 
					: ($constantThemeNum=81)
						If ($constantNum>300)
