...
$(function()
{
    // GESTION DES FAQ
  initajaxpage ();
  var pageasadia = $("#pageasadia").val();
  gerepages(pageasadia);
  function gerepages(page)
    {
      // Page DIAGNOSE
      switch (page) {
        case "P1":   
          gerepagediagnose ();
          stopPropagation(page)
        break;
        ...
        case "P8":
         ...
         /*  LISTE DES organes sains */
        case "P9":
          gerepageorganessains ();
          stopPropagation(page);
          break;
    };
     initajaxpage();
  ...
  function initajaxpage (){
    ...
    $('[id^="Ajax"]').click (function()
    {
      ...
      var nompage = reponse.substring(0,poslivre);
      document.title = nompage;
      reponse = reponse.substring(poslivre+1);
      var poslivre = reponse.indexOf("£");
      var numpage = reponse.substring(0,poslivre);
      $('#pageasadia').val(numpage);
      reponse = reponse.substring(poslivre+1);
      $('#corps').html(reponse);
      gerepages(numpage);
   });
  ...
  function gerepagediagnose()    {
      /*  DIAGNOSE : Masquage intitial des résultats de recherche  */
      $('#resultat').hide();
      $('#critere').hide();
      $('#formule').hide();
      $('#boutonvoir').hide();
      ...
      $('#trouveparnum').change(function() // Clic sur la recherche d'une photographie
      {
      envoirequetedia();
      });
      changedeliste ();
  ...
  function clicsuriconedetailphoto(lui)
    {
      ...
      if (panier)
      {  // cas du clic sur le bouton "panier" des images en liste
        ...
        if (NbPhotos>200) {
         ...
         /* Gestion du clic sur le bouton de voir les photos #bouvisenombreuse */
          $('#bouvisenombreuse').click(function(){
            var requete = choixNavigateur();
            var doc = "diaflipfloc"+liste+formule;
            requete.open("post",doc,false);
            requete.send(null);
            var reponse = requete.responseText;
            var pos = reponse.indexOf("£");
 ...
 function tuelarecherche()
  ...
  function changedeliste ()
    {
      /* Gestion de la liste anapath */
        ...
        $('#asa').on('click', function()
          {
            var gibier = $("#typevisiteur").val();
            var json = '{"nomenclature" : this.id,"typevisiteur" : "'+gibier+'"}'
            var doc = 'nomenclature'+json;
            var requete = choixNavigateur();
            requete.open("post",doc,false);
            requete.send(null);
  ...
  function affichelesphotos(une)
    {
      ...
      if (une.length==5)
        ...
        { /* "toutes" = les images du tableau affichée  */
      ...
      /* gestion du clic de l'image de choix d'une explication toutes les images de choix d'une explication */
      /* ont une id de type lp+ x= type d'explication + n°de la photo + rang donc commençant par lp */
      $('[id^="lp"]').click(function(){
        clicsuriconedetailphoto(this);
      });
      $('[id^="boutonnpc"]').click(function(){
  ...
  function engrand(lui)
    ...
    function gerephotoengrand(){
      ...
      /* FGERMETURE des éléments d'affichage*/
       $('#com').hide();
       $('#fermeture').hide();
     ...
     /* AFFICHAGE DES PHOTOS LIEES  */
       $('[id^="imagette"]').click(function(){
         ...
         $(this).attr('src',"PhotosAsaDia/"+varancnum+"is.jpg");/* source  */
        
         /* Modification de l'image */
           var srcimagea = 'PhotosAsaDia/'+numimagea+'.jpg';
     ...
     /*  METTRE ENLEVER LES FLECHES */
      ...
      // gestion de la LISTE DES MOTIFS
  function gerepagemotif (){
    formatetb('#listemotif');
    initclicdetailmotif();
  ...
  /*  GESTION DE LA LISTE DES LESIONS  */
  function gerepagelesion (){
    ...
    function initclicdetaillesion(){
     ...
     /*  GESTION DE LA LISTE DES PHOTOS  */ 
  function gerepagephoto (){
    formatetb('#listephoto')
    initclicdetailphoto();
  ...
  function gerepageorganessains (){
    $('[id^="anat"').click(function(){
      var typeclic = this.id.substring(4,5);
      var idespece = $('input[name="espece"]:checked').attr('id');
      var idorgane = $('input[name="organe"]:checked').attr('id');
      var requete = choixNavigateur();/* envoi du message AJAX  à 4D  */
      var doc = 'anatomie'+"_"+idespece+"_"+idorgane;
      requete.open("post",doc,false);
      requete.send(null); /* traitée par la méthode DiaWebAbouleModifOrganeSain de 4D  */
      var reponse = requete.responseText;
      var pos = reponse.indexOf('£');
      if (typeclic=="e") {
      $('#organe').html(reponse.substring(0,pos));
      }
      $('#pavephotos').html(reponse.substring(pos+1));
      $('[id^="anatphoto"]').click(function(){
        clicphotoanat(this);
      });
      $('.photoasadia').click(function(){ /* gestion du clic de l'image de la lésion */
        engrandanat(this);
  ...
  function clicphotoanat(lui){
    var srcimageclic = $(lui).attr('src');
    var srcimageclicgrande = srcimageclic.replace('is.jpg','i.jpg');
    var srcimagegrande = $('#grandephotoanat').attr('src');
    var srcimagegrandepetite = srcimagegrande.replace('i.jpg','is.jpg');
    var comimageclic = $(lui).attr('com');
    var comimagegrande = $('#grandephotoanat').attr('com');
    var altimageclic = $(lui).attr('alt');
    var altimagegrande = $('#grandephotoanat').attr('alt');
    $('#grandephotoanat').attr('src',srcimageclicgrande);
    $('#grandephotoanat').attr('com',comimageclic);
    $('#grandephotoanat').attr('alt',altimageclic);
    $("#"+lui.id).attr('src',srcimagegrandepetite);
    $("#"+lui.id).attr('com',comimagegrande);
    $("#"+lui.id).attr('alt',altimagegrande);
    $('#comphoto').html(comimageclic);
    $("#com"+lui.id).html(altimagegrande);
    $('[id^="anatphoto"]').click(function(){
      clicphotoanat(this);
    });
    $('.photoasadia').click(function(){ /* gestion du clic de l'image de la lésion */
  ...
  function engrandanat(lui){
    var srcimage = $(lui).attr('src');
    var largeurf = $(window).width(); /* Largeur de la fenetre du navigateur */
    var hauteurf = $(window).height();/* Hauteur de la fenetre du navigateur */
    var doc = 'imageengrandanat£'+srcimage+"£"+largeurf+"£"+hauteurf;/* fabrication du message AJAX envoyé à 4D  */
    var requete = choixNavigateur();/* envoi du message AJAX  à 4D  */
    requete.open("post",doc,false);
    requete.send(null); /* traitée par la méthode WebAfficheImageEnGrand de 4D  */
    var reponse = requete.responseText;
  ...
  // GESTION DES PANIERS
  function gerepagepanier(){
    initclicdetailpanier();
    initclicpaniervoir();
    initclicpanierpartage();
  ...
  function initclicpaniervoir(){
    $('[id^="paniervoir"]').click(function(){
      ...
      $('#paniertuer').click(function(){
        ...
        requete.send(null); /* traitée par la méthode aboulelistelesion de 4D  */
        var reponse = requete.responseText;
        /* type Oui£Non£Supprimer le panier£Vous avez demandé à supprimer le panier. <br />Si vous persistez, le panier n'apparaîtra plus dans la liste. <br />Notez toutefois que vous pourrez demander à l'ASA de le restituer. <br /> <br />Vouler vous VRAIMENT supprimer le panier ???  */
        var pos = reponse.indexOf('£');
...
// gestion de l'envoi du panier pour le partager
  function initclicpanierpartage(){
...
// masquage initial du détail du panier    
  ...
  function initclicauteurspumpanier(){
    $('[id="auteurspumpanier').click(function(){
      ...
      reponse = reponse.substring(pos+1);
      $('#nbactuel').val(nb);
      $('#listepanier').html(reponse);
      formatetb('#listepanier');
      initclicdetailpanier();
      initclicpaniervoir()
      initclicpanierpartage()
      $('#colonnepanier').hide();
  ...
  function gerepagequizz(){
    $('[id^="quizzlancer"]').click(function(){
      var doc = this.id + '£'+$('#mailvisiteur').val();
      var requete = choixNavigateur();/* envoi du message AJAX  à 4D  */
      requete.open("post",doc,false);
      requete.send(null);
      var reponse = requete.responseText;
